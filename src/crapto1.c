 ...
 */
struct Crypto1State*
lfsr_common_prefix(uint32_t pfx, uint32_t rr, uint8_t ks[8], uint8_t par[8][8], uint8_t uid, uint8_t nt)
{
	struct Crypto1State *statelist, *s;
	uint32_t *odd, *even, *o, *e, top, lastOdd;
		...
		for(e = even; *e + 1; ++e)
			for(top = 0; top < 64; ++top) {
				...
				*e += (!(top & 7) + 1) << 21;
				...
				lastOdd = state->odd = statelist->odd;
				state->even = statelist->even;
